...
func (a *App) Auth(ctx context.Context) string {
	...
	qr, err := a.client.Authenticate(ctx)
	if err != nil {
...
func (a *App) Status(ctx context.Context) string {
	...
	if err := a.client.SimpleConnect(ctx); err != nil {
		return output.Error(err)
	}

	// Wait up to 3 seconds for connection to be fully established
	for i := 0; i < 6; i++ {
	...
	if !a.client.IsConnected() {
		return output.Success(map[string]interface{}{
			"status":        "disconnected",
			"authenticated": true, // credentials exist but connection failed
		})
	}
...
func (a *App) GetProfile(ctx context.Context) string {
	if err := a.client.Connect(ctx); err != nil {
		return output.Error(err)
	}

	info, err := a.client.GetProfileInfo(ctx)
	if err != nil {
		return output.Error(err)
	}

	jid, _ := types.ParseJID(info["jid"].(string))
	pictureURL, _ := a.client.GetProfilePicture(ctx, jid)
...
func (a *App) SendPresence(ctx context.Context, recipient string, isTyping bool) string {
	if err := a.client.Connect(ctx); err != nil {
		return output.Error(err)
	}

	if err := a.client.SendPresence(ctx, recipient, isTyping); err != nil {
		return output.Error(err)
	}
...
func (a *App) CheckNumber(ctx context.Context, phone string) string {
	if err := a.client.Connect(ctx); err != nil {
		return output.Error(err)
	}

	results, err := a.client.IsOnWhatsApp(ctx, []string{phone})
	if err != nil {
		return output.Error(err)
	}
...
func (a *App) Listen(ctx context.Context, redisURL, userID string) string {
	if err := a.client.EnableRedis(redisURL, "whatsapp:events:"+userID); err != nil {
		return output.Error(err)
	}

	fmt.Fprintf(os.Stderr, "🚀 Interaction listener started for user %s on Redis %s\n", userID, redisURL)
	return a.Sync(ctx)
}
...
func (a *App) Sync(ctx context.Context) string {
	...
	eventHandler := func(evt interface{}) {
		...
		case *events.Message:
			...
			// Real-time Push to Redis (Event-driven interaction)
			a.client.PublishEvent(ctx, "message", map[string]interface{}{
				"id":        id,
				"phone":     sender,
				"chat_jid":  chatJID,
				"message":   content,
